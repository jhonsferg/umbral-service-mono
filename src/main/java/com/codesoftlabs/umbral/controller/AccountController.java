...
public class AccountController {
    ...
    public Account create(@Valid @RequestBody CreateAccountDto dto) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved all accounts", content = @Content(schema = @Schema(implementation = Account.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized - authentication required"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public List<Account> findAll(@RequestAttribute("userId") UUID userId) {
    ...
    public Account findOne(@RequestAttribute("userId") UUID userId, @PathVariable @Parameter(description = "Account ID") UUID id) {
        return accountService.findOne(userId, id);
    }

    @PatchMapping("/{id}")
    @Operation(summary = "Update an account", description = "Updates an existing account with new information")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Account updated successfully", content = @Content(schema = @Schema(implementation = Account.class))),
            @ApiResponse(responseCode = "400", description = "Invalid input provided"),
            @ApiResponse(responseCode = "401", description = "Unauthorized - authentication required"),
            @ApiResponse(responseCode = "403", description = "Forbidden - insufficient permissions"),
            @ApiResponse(responseCode = "404", description = "Account not found"),
    ...
    public Account update(@RequestAttribute("userId") UUID userId, @PathVariable @Parameter(description = "Account ID") UUID id,
                          @Valid @RequestBody UpdateAccountDto dto) {
        return accountService.update(userId, id, dto);
    }
    ...
    public void remove(@RequestAttribute("userId") UUID userId, @PathVariable @Parameter(description = "Account ID") UUID id) {

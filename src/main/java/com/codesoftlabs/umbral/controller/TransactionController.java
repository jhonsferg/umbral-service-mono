...
public class TransactionController {
    ...
    public Transaction create(@RequestAttribute("userId") UUID userId,
        ...
        return transactionService.create(userId, createTransactionDto, file);
    }
    ...
    public PaginatedResponseDto<Transaction> findAll(@RequestAttribute("userId") UUID userId,
        ...
        return transactionService.findAll(userId, type, categoryId, accountId, startDate, endDate, search, page, limit);
    }
    ...
    public Map<String, Object> getBalanceSummary(@RequestAttribute("userId") UUID userId) {
    ...
    public List<Map<String, Object>> getCategorySummary(@RequestAttribute("userId") UUID userId) {
    ...
    public List<Map<String, Object>> getMonthlyTrend(@RequestAttribute("userId") UUID userId,
                                                     @RequestParam(required = false) @Parameter(description = "Time range for trend analysis (e.g., 6m, 1y)") String range) {
        return transactionService.getMonthlyTrend(userId, range);
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get a specific transaction", description = "Retrieves detailed information about a specific transaction")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Transaction retrieved successfully", content = @Content(schema = @Schema(implementation = Transaction.class))),
            @ApiResponse(responseCode = "401", description = "Unauthorized - authentication required"),
            @ApiResponse(responseCode = "403", description = "Forbidden - insufficient permissions"),
            @ApiResponse(responseCode = "404", description = "Transaction not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public Transaction findOne(@RequestAttribute("userId") UUID userId, @PathVariable @Parameter(description = "Transaction ID") UUID id) {
        return transactionService.findOne(userId, id);
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Transaction updated successfully", content = @Content(schema = @Schema(implementation = Transaction.class))),
            @ApiResponse(responseCode = "400", description = "Invalid input provided"),
            @ApiResponse(responseCode = "401", description = "Unauthorized - authentication required"),
            @ApiResponse(responseCode = "403", description = "Forbidden - insufficient permissions"),
            @ApiResponse(responseCode = "404", description = "Transaction not found"),
    ...
    public Transaction update(@RequestAttribute("userId") UUID userId,
                              @PathVariable @Parameter(description = "Transaction ID") UUID id,
                              @Valid @ModelAttribute UpdateTransactionDto updateTransactionDto,
                              @RequestPart(value = "receipt", required = false) MultipartFile file) {
        return transactionService.update(userId, id, updateTransactionDto, file);
    }
    ...
    public void remove(@RequestAttribute("userId") UUID userId, @PathVariable @Parameter(description = "Transaction ID") UUID id) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Receipt uploaded successfully", content = @Content(schema = @Schema(implementation = Transaction.class))),
    ...
    public Transaction uploadReceipt(@RequestAttribute("userId") UUID userId,
                                     @PathVariable @Parameter(description = "Transaction ID") UUID id,
                                     @RequestPart("receipt") MultipartFile file) {
        return transactionService.update(userId, id, new UpdateTransactionDto(), file);
    }
}
